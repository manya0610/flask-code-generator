...
from flask_code_generator.controller_generator import ControllerGenerator
from flask_code_generator.database_model import DataBaseModel
from flask_code_generator.repo_generator import RepoGenerator
from flask_code_generator.service_generator import ServiceGenerator
...
class CRUDGenerator:
    ...
    def __init__(
        ...
        self.database_model = DataBaseModel(
            self.project_name, model_name, model_attributes, models_file
        )
        ...
        self.controller_generator = ControllerGenerator(
            self.database_model, self.service_file
        )
        self.controller_file = (
            self.database_model.model_name_snake_case + "_" + "controller"
        )

    def repo_file_generator(self) -> None:
        with open(
            f"{self.project_name}/{constants.REPO_FOLDER}/{self.repo_file}.py",
            "w",
            encoding="utf-8",
        ) as f:
    ...
    def service_file_generator(self) -> None:
        with open(
            f"{self.project_name}/{constants.SERVICE_FOLDER}/{self.service_file}.py",
            "w",
            encoding="utf-8",
        ) as f:
    ...
    def controller_file_generator(self) -> None:
        with open(
            f"{self.project_name}/{constants.CONTROLLER_FOLDER}/{self.controller_file}.py",
            "w",
            encoding="utf-8",
        ) as f:
