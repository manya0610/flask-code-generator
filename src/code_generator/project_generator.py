...
class ProjectGenerator:
    ...
    def constants_file_generator(self) -> None:
        with open(f"{self.project_name}/{constants.CONSTANTS_FOLDER}/{constants.CONSTANTS_ERROR_MESSAGE_FILE}.py", "w", encoding="utf-8") as f:
            f.write(self.constant_generator.error_message_constants_generator())

    def database_file_generator(self) -> None:
        with open(f"{self.project_name}/{constants.DATABASE_FOLDER}/{constants.DATABASE_FILE}.py", "w", encoding="utf-8") as f:
